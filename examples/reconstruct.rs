...
fn main() -> Result<(), trackable::error::MainError> {
    ...
    let matches = App::new("reconstruct")
        .arg(
            ...
            Arg::with_name("MISSING_FRAGMENT")
                .short("d")
                .takes_value(true),
        )
        .get_matches();
    ...
    let d = track_any_err!(matches.value_of("MISSING_FRAGMENT").unwrap().parse())?;
    ...
    let data = track!(ec.reconstruct(d, &inputs2[..]))?;
